                            ...
                            foreach (Entities.Client c in client)
                            {
                                ...
                                if (clientId > 0)
                                {
                                    foreach (Entities.ClientAddress ca in c.ClientAddressess)
                                    {
                                        ...
                                        {
                                            if (ca.CustomerAndTransporterMapping != null)
                                            {
                                                if (ca.CustomerAndTransporterMapping.Count > 0)
                                                {
                                                    // Loop through customer address and transporter mapping list
                                                    foreach (Entities.CustomerAndTransporterMapping ctm in ca.CustomerAndTransporterMapping)
                                                    {
                                                        ctm.CustomerAddressId = clientAddressId;

                                                        CustomerAndTransporterMapping ctmDL = new CustomerAndTransporterMapping();

                                                        var mappingId = ctmDL.SaveCustomerAndTransporterMapping(ctm, transaction);
                                                    }
                                                }
                                            }
                                        }
