...
namespace SharpiTech.POS.DataModel
{
    public class Role
    {
        ...
        public Entities.Role GetRoleByName(string roleName)
        {
            var role = new Entities.Role();

            DbCommand dbCommand = null;

            using (dbCommand = database.GetStoredProcCommand(DBStoredProcedure.GetRoleByName))
            {
        ...
        /// <returns>An object representing the role</returns>
        public Entities.Role GetRoleById(Int32 roleId)
        {
            var role = new Entities.Role();

            DbCommand dbCommand = null;

            using (dbCommand = database.GetStoredProcCommand(DBStoredProcedure.GetRoleById))
            {
        ...
        public List<Entities.Role> GetAllRoles()
        {
            ...
            try
            {
                using (dbCommand = database.GetStoredProcCommand(DBStoredProcedure.GetAllRoles))
                {
                    using (IDataReader reader = database.ExecuteReader(dbCommand))
                    {
                        while (reader.Read())
